...
export const solve = () => {
  ...
  for (i = 0; i < myExpenseReport.length; i++) {
    for (j = 0; j < myExpenseReport.length; j++) {
      if (myExpenseReport[i] + myExpenseReport[j] === 2020) {
        console.log(`The winners are ${myExpenseReport[i]} and ${myExpenseReport[j]}!`)
        console.log('The answer is: ', myExpenseReport[i] * myExpenseReport[j])
        return myExpenseReport[i] * myExpenseReport[j]
      }
    }
  }
}
